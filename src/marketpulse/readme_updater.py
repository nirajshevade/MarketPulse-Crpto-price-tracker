...
from .config import COIN_LABELS


def _render_price_rows(
    ...
    for coin in current_prices:
        rows.append(
            "| "
            f"{COIN_LABELS.get(coin, coin)} | "
            f"${current_prices[coin]:,.4f} | "
            f"{api_24h_changes.get(coin, 0.0):+.2f}% | "
            f"{run_to_run_changes.get(coin, 0.0):+.2f}% |"
        )
    return "\n".join(rows)
...
def build_readme_content(
    ...
    rows = _render_price_rows(current_prices, api_24h_changes, run_to_run_changes)

    return f"""# MarketPulse - Crypto Price Tracker Automation
...
- Alert threshold: **+/-{threshold:.2f}%** run-to-run movement
